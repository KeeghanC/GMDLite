...
//---
void GridWorld::DstarDisplayPath(int x, int y){
	vertex *currentVertex = &map[y][x];
	// std::cout << "test" << std::endl;
	vertex *neighbour;
	vertex *min_neighbour;
	// std::cout << "(" << to_string(currentVertex->col) << ","  << currentVertex->row << ") -> ";
	// std::cout << "Goal vertex = " << goalVertex.col << ", " << goalVertex.row << std::endl;
	double min_g_plus_c = INF;
	int i =0;


	// std::cout << "currentVertex->col: " << currentVertex->col << std::endl;
	// std::cout << "currentVertex->row: " << currentVertex->row << std::endl;
	// std::cout << "goalVertex.col: " << goalVertex.col << std::endl;
	// std::cout << "goalVertex.row: " << goalVertex.row << std::endl;

	// std::cout << currentVertex->col << " && " << "==" << goalVertex.row << std::endl;
	// while (!((currentVertex->col == goalVertex.col) && (currentVertex->row == goalVertex.row))){
	while (!((currentVertex->col == goalVertex.col) && (currentVertex->row == goalVertex.row))){
		...
		// std::cout << "Going to " << min_neighbour->col << " " << min_neighbour->row << std::endl;
		// std::cout << "(" << to_string(min_neighbour->col) << ","  << min_neighbour->row << ") -> ";
		setcolor(RED);
		...
		if (min_neighbour == &map[startVertex.row][startVertex.col]) {
			break;
		}
...
string GridWorld::LPAdisplayPath()
{
	...
	vertex *neighbour;
	vertex *min_neighbour;
	// std::cout << "Goal node RHS: " << currentVertex->rhs << std::endl;
	//currentVertex->move

	string output = to_string(currentVertex->row) + to_string(currentVertex->col);
	double min_g_plus_c = INF;
	while (1)
	{
		...
		output += " <- " + to_string(min_neighbour->row) + to_string(min_neighbour->col);
		setcolor(RED);
...
string GridWorld::cellName(vertex * item){
	string result = "";
	result += letterY(item);
...
char GridWorld::letterY(vertex *item){
	int y = item->row - 1;
